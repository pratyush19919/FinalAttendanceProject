from flask import Flask, flash,send_file, render_template,url_for,redirect,request,Response
...
import time
from face_track import TrackImages, image
import cv2
import numpy as np
import os
...
class User(db.Model):
    ...
    semesters = db.Column(db.String(120))
        ...
        if user:
            if user.password == form.password.data:
                if user.username == "admin":
                    ...
                    flash("Hi! " + form.username.data + " You have successfully logged in") 
                    sems = user.semesters
                    sems = sems.split(",")
                    print(user)
                    return render_template("teacher.html",semesters = sems,user=user.id)
            else:
...
def signup():
    ...
    if request.method == "POST":
        if form.validate_on_submit(): 
            semlist=request.form.getlist("semval")
            semstr = ','.join(semlist)
        # return "<h1>" + form.username.data + " " + form.email.data +" " +  form.password.data + "</h1>"
        new_user = User(username=form.username.data,email = form.email.data, password = form.password.data,semesters=semstr)
...
def train_face():
    ...
    flash("It's done now add new student otherwise logout.")
    return render_template('admin.html',name = name,id="id_student")
...
def teacher():
    ...
    reqUser = User.query.filter_by(id=int(user)).first()
    sems = reqUser.semesters         
    sems = sems.split(",")

    return render_template("teacher.html",user=user,semesters=sems)
...
def class1():
    ...
    datestr = []
    for i in date_att:
        ppl.append(len(list(Update_form.query.filter_by(date_created=str(i)))))
      
    for i in date_att:
        datestr.append(str(i))
    r=[]
    for i in datestr:
        l = i[0:19]
        r.append(l)
    ...
    return render_template("class1.html",date_att_prev = date_att_prev,imgstr=imgstr,user=user)
...
@app.route("/take_att",methods=["GET","POST"])
def take_att():
    ...
    return render_template("class1.html",date_att=date_att,imgstr=imgstr,user=user)
...
def take_att2():
    ...
    return render_template("class2.html",date_att=date_att)
